<?php
    ...
    if (!$sitenameProfile = SitesProfile::getSiteName($sitename)) {
        echo '<div class="cbp-l-project-title">This Page does not exit!!!</div>';
        exit();
    }

    $mysitename = $sitenameProfile->getValueEncoded("sitename");
    $image1 = $sitenameProfile->getValueEncoded("image1");
    $image2 = $sitenameProfile->getValueEncoded("image2");
    $image3 = $sitenameProfile->getValueEncoded("image3");
    $description = $sitenameProfile->getValueEncoded2("description");
    $client = $sitenameProfile->getValueEncoded("client");
    $datedelivered = $sitenameProfile->getValueEncoded("datedelivered");
    $webaddress = $sitenameProfile->getValueEncoded2("webaddress");

    $categoryid = $sitenameProfile->getValueEncoded("categoryid");

    // Category Class
    $categorytitle2 = SiteCategory::getCategoryTitle($categoryid);
    $categorytitle = $categorytitle2->getValueEncoded("title");
?>
<style type="text/css">
    a:hover.cbp-l-project-related-link {text-decoration:none;}
</style>
<div class="cbp-l-project-title"><?php echo $mysitename; ?></div>
<div class="cbp-l-project-subtitle">by Adebayo Peter Olaonipekun</div>

<div class="cbp-slider" style="border: 1px solid rgba(36, 59, 123, 0.1);">
    <ul class="cbp-slider-wrap">
        <li class="cbp-slider-item">
            <a href="<?php echo BASE_URL . DOC_UPLOADPATH_SITE_PROFILE_1200_900 . $image1; ?>" class="cbp-lightbox">
                <img src="<?php echo BASE_URL . DOC_UPLOADPATH_SITE_PROFILE_1200_900 . $image1; ?>" alt="<?php echo $mysitename; ?>">
            </a>
        </li>

        <?php if ($image2) { // check if it exsits ?>
        <li class="cbp-slider-item">
            <a href="<?php echo BASE_URL . DOC_UPLOADPATH_SITE_PROFILE_1200_900 . $image2; ?>" class="cbp-lightbox">
                <img src="<?php echo BASE_URL . DOC_UPLOADPATH_SITE_PROFILE_1200_900 . $image2; ?>" alt="<?php echo $mysitename; ?>">
            </a>
        </li>
        <?php } // show if it exit ?>

        <?php if ($image3) { // check if it exsits ?>
        <li class="cbp-slider-item">
            <a href="<?php echo BASE_URL . DOC_UPLOADPATH_SITE_PROFILE_1200_900 . $image3; ?>" class="cbp-lightbox">
                <img src="<?php echo BASE_URL . DOC_UPLOADPATH_SITE_PROFILE_1200_900 . $image3; ?>" alt="<?php echo $mysitename; ?>">
            </a>
        </li>
        <?php } // show if it exit ?>
    </ul>
...
<div class="cbp-l-project-container">
    <div class="cbp-l-project-desc">
        <div class="cbp-l-project-desc-text"><?php echo $description; ?></div>
    ...
    <div class="cbp-l-project-details">
        <ul class="cbp-l-project-details-list">
            ...
            <li><strong>Date</strong><?php echo date('d F, Y', strtotime($datedelivered)); ?></li>
            <li><strong>Categories</strong><?php echo $categorytitle; ?></li>
        </ul>
        <a href="<?php echo $answer = str_ireplace("http//","http://",$webaddress); ?>" target="_blank" class="cbp-l-project-details-visit">OPEN PROJECT</a>
    </div>
</div>
...
<div class="cbp-l-project-container">
    <div class="cbp-l-project-related">
        ...
        <ul class="cbp-l-project-related-wrap">
            <?php
                // Get 3 recent related project apart from the one shown already
            $rCategoryId = $categoryid;
            $rSitename = $mysitename;
            $rStartRow = 0;
            $rNumRows = 3;

            $relatedProjects = SitesProfile::getRelatedProject($rCategoryId, $rSitename, $rStartRow, $rNumRows);

            foreach ($relatedProjects as $relatedProject) {
                $rpSiteName = $relatedProject->getValueEncoded("sitename");
                $rpImage1 = $relatedProject->getValueEncoded("image1");
            ?>

            <li class="cbp-l-project-related-item">
                <a href="<?php echo BASE_URL; ?>projects/projects.php?skd=<?php echo $rpSiteName; ?>" class="cbp-singlePage cbp-l-project-related-link" rel="nofollow">
                    <img src="<?php echo BASE_URL . DOC_UPLOADPATH_SITE_PROFILE_380_250 . $rpImage1; ?>" alt="<?php echo $rpSiteName; ?>">
                    <div class="cbp-l-project-related-title"><?php echo $rpSiteName; ?></div>
                </a>
            </li>
            <?php } //  end of foreach loop : this loop only generate 3 related projects ?>
        </ul>
