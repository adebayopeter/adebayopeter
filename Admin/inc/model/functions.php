<?php
	...
	function getProductID() {
		require(ROOT_PATH . 'inc/database_convar.php');
		$sqlproductid = "SELECT max(sku) AS 'sku' FROM product_tbl";
		$selectproductid = $db->query($sqlproductid);
		$prod_id = $selectproductid->fetch();
		$product_id = $prod_id['sku'];
		if (!empty($product_id)) {
	...
	function getFacebookURL() {
		require(ROOT_PATH . 'inc/database_convar.php');
		$sql = "SELECT url FROM social_media_tbl WHERE social_media = 'Facebook'";
		$selectsoc = $db->query($sql);
		$url = $selectsoc->fetch();
		$social_media_url = $url['url'];
	...
	function getTwitterURL() {
		require(ROOT_PATH . 'inc/database_convar.php');
		$sql = "SELECT url FROM social_media_tbl WHERE social_media = 'Twitter'";
		$selectsoc = $db->query($sql);
		$url = $selectsoc->fetch();
		$social_media_url = $url['url'];
	...
	function getYouTubeURL() {
		require(ROOT_PATH . 'inc/database_convar.php');
		$sql = "SELECT url FROM social_media_tbl WHERE social_media = 'YouTube'";
		$selectsoc = $db->query($sql);
		$url = $selectsoc->fetch();
		$social_media_url = $url['url'];
	...
	function getInstagramURL() {
		require(ROOT_PATH . 'inc/database_convar.php');
		$sql = "SELECT url FROM social_media_tbl WHERE social_media = 'Instagram'";
		$selectsoc = $db->query($sql);
		$url = $selectsoc->fetch();
		$social_media_url = $url['url'];
	...
	function getUsersDetails($id, $usercode) {
		require(ROOT_PATH . 'inc/database_convar.php');
		$sql = "SELECT username,firstname,lastname,image,occupation,role,rolecategory FROM users_tbl WHERE id = '$id' AND usercode = '$usercode'";
		$selectuser = $db->query($sql);
	...
	function create_image_dimenion($target, $width, $height, $target_thumb) {	

		// Code for dimension begins here
		...
		if ($img) {
			...
			imagejpeg($img,$target_thumb);
			
		}// End code for dimension creating
	}